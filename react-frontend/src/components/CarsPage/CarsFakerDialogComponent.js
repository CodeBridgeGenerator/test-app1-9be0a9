...
import carsFakerFactory from "./carsFakerFactory";
...
const CarsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = carsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cars").create(elem));
...
export default connect(mapState, mapDispatch)(CarsFakerDialogComponent);
