...
const CarsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.brand}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "cars"});
...
<Column field="brand" header="Brand" body={pTemplate1} filter={selectedFilterFields.includes("brand")} hidden={selectedHideFields?.includes("brand")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload Cars Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="cars"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Cars" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
